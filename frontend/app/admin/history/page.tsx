...
export default function HistoryPage() {
  const { data: history = [], isLoading, error } = useSWR<HistoryEntry[]>('/reservations/history', fetcher);
          ...
          <table className="w-full border border-gray-200">
            <thead>
              <tr className="bg-white">
                ...
                <th className="text-left p-3 md:p-4 border border-gray-200 font-semibold text-sm md:text-base">Username</th>
            ...
            <tbody>
              {history.map((h) => (
                <tr key={h.id}>
                  ...
                  <td className="p-3 md:p-4 border border-gray-200 text-sm md:text-base">{h.user?.username ?? 'Deleted user'}</td>
